...
Cylinder::Cylinder(float baseRadius, float topRadius, float height, int nslices, int nstacks, float red, float green, float blue)
{
    // tan(angle) to compute the radius of each stack
    float tantheta = fabs((baseRadius-topRadius)/height);
    float radius;
    ...
    for (int j =0; j<nstacks; j++){
        ...
        if (baseRadius>topRadius) {
            ...
            radius = topRadius-(height*j/(float)(nstacks-1))*tantheta;
        }
...
void Cylinder::draw(float* local2clip, float* local2eye, float* world2eye, GLuint posHandler, GLuint colorHandler, GLuint normalHandler, GLuint local2clipHandler, GLuint local2eyeHandler, GLuint world2eyeHandler, GLuint normalMatHandler)
{
